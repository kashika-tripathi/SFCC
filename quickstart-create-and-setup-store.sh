...
sfdx community create --name "$storename" --template-name "B2B Commerce (LWR)" --url-path-prefix "$storename" --description "Store $storename created by Quick Start script."
echo ""
...
do
    ...
    storeId=$(sfdx data query -q "SELECT Id FROM WebStore WHERE Name='${storename}' LIMIT 1" -r csv |tail -n +2)
    sleep 10
done
